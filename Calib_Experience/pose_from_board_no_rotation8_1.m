%%测试 使用像素误差版 RANSAC PnP  只有真实环境   加坐标轴

function [C_B, R_CW, T_BC, dbg,roll_deg, pitch_deg, yaw_deg] = pose_from_board_no_rotation8_1(calib_data, kk, squareSize, use_corner_find,opts)
...
R_WC0 = R_CW.';
zc0 = R_WC0(:,3);
fprintf('[Before flip] zc_world = [%.6f %.6f %.6f]\n', zc0(1), zc0(2), zc0(3));
disp('R_CW before ='); disp(R_CW);
disp('R_WC before ='); disp(R_WC0);
...
if didFlip
    Rflip = [ 1  0  0;
              0 -1  0;
              0  0 -1 ];  % Rx(pi)  <--- 方案A
    disp('Rflip used ='); disp(Rflip);

    R_CW = Rflip * R_CW;

    % 先别镜像位置，避免混淆（你之后再决定加不加）
    % C_B(3) = -C_B(3);
end
R_WC = R_CW.';   % <<< 【必须】保证后续全部使用翻转后的 R_WC
zc_world = R_WC(:,3);
...
R_WC1 = R_CW.';
zc1 = R_WC1(:,3);
fprintf('[After  flip] didFlip=%d, zc_world = [%.6f %.6f %.6f]\n', didFlip, zc1(1), zc1(2), zc1(3));
disp('R_CW after  ='); disp(R_CW);
disp('R_WC after  ='); disp(R_WC1);

% rotation change magnitude (deg)
Rdiff = R_WC1 * R_WC0';
ang = acos(max(-1,min(1,(trace(Rdiff)-1)/2)));
fprintf('Rotation difference = %.6f deg\n', rad2deg(ang));


%%% ===================== MOD-1: 翻转逻辑提前，并且翻转后立刻重算 R_WC =====================
if zc_world(3) > 0
    % === flip about camera X by 180deg: Rx(pi) ===
Rflip = [ 1  0  0;
          0 -1  0;
          0  0 -1 ];

R_CW = Rflip * R_CW;   % 等价于 R_WC = R_WC * Rflip
C_B(3) = -C_B(3);      % 如果你确实需要把相机“镜像到平面另一侧”，保留；否则可先注释掉验证
...
%%% ===================== MOD-2: 欧拉角必须在最终 R_WC 上计算 =====================
% 已知：R_WC 是 camera->world 的旋转矩阵
% R_WC: camera -> world rotation matrix

eulZYX = rotm2eul(R_WC, 'ZYX');   % [yaw pitch roll] (rad)
...
%%% ===================== MOD-3: 后续 heading / 投影 也必须用更新后的 zc_world =====================
zproj    = zc_world - dot(zc_world,[0;0;1])*[0;0;1];   % 投影到Z_B=0
if norm(zproj) > 1e-12
    zproj = zproj / norm(zproj);
    heading_deg = atan2d(zproj(2), zproj(1));          % 从+X_B逆时针为正
    theta_x_deg = acosd( max(-1,min(1, dot(zproj,[1;0;0]) )) );
    theta_y_deg = acosd( max(-1,min(1, dot(zproj,[0;1;0]) )) );
else
...
%% 打印结果2
%eulZYX = rad2deg(rotm2eul(R_WC, 'ZYX')); % MATLAB自带
%eulZXY = rad2deg(rotm2eul(R_WC, 'ZXY'));
%fprintf('rotm2eul ZYX = [%.2f %.2f %.2f]\n', eulZYX);
%fprintf('rotm2eul ZXY = [%.2f %.2f %.2f]\n', eulZXY);
%% 打印结果3
%Rdiff = R_WC_after * R_WC_before';
%ang = acos(max(-1,min(1,(trace(Rdiff)-1)/2)));
%fprintf('Rotation difference = %.6f deg\n', rad2deg(ang));
%%
...
fprintf('RANSAC finished after %d iterations (maxTrials=%d)\n', iterCount, opts.maxTrials);

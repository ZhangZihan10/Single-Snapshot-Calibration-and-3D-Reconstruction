%% AGHP-RRT星 路径规划 不成功 结合鱼眼相机重构系统结果(真实环境）改变逆运动学解
%clc; clear;
...
numberTran3(arduino,path(10,1),path(10,2),path(10,3),path(10,4));
%pause(2);
numberTran3(arduino,path(20,1),path(20,2),path(20,3),path(20,4));
%pause(2);
numberTran3(arduino,path(30,1),path(30,2),path(30,3),path(30,4));
%pause(2);
numberTran3(arduino,path(40,1),path(40,2),path(40,3),path(40,4));
%pause(2);
numberTran3(arduino,path(50,1),path(50,2),path(50,3),path(50,4));
%pause(2);
numberTran3(arduino,path(60,1),path(60,2),path(60,3),path(60,4));
%pause(2);
numberTran3(arduino,path(70,1),path(70,2),path(70,3),path(70,4));
%pause(2);
...
function [path, tree] = aghp_rrt_star_planning(robot, q_start, q_goal, obstacles, t, rho2_ratio, rho3_ratio, max_iter)
   ...
   % dim = 4;            % 有效维度
    ...
    for i = 1:max_iter
        ...
        % --- 在函数开头定义权重 ---
        W = [20.0, 1.0, 1.0, 1.0]; % 赋予 Z 轴极高的权重
        ...
        if ~checkCollision2(robot, q_near, q_new, obstacles)
            ...
            W = [15.0, 1.0, 1.0, 1.0];
            min_cost = tree(idx_near).cost + norm((q_new - q_near) .* W);
            ...
            for j = 1:length(neighbors)
                ...
                cost_through_new = min_cost + norm((neighbor - q_new) .* W);

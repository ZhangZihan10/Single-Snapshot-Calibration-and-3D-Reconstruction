...
filename_laser = 'BoxL/ObstacleBoardL_1.jpg'; % 用于提取基准底边
filename_front = 'BoxR/ObstacleBoard_1.jpg';       % 正面抠图文件（请修改为你的实际路径）
filename_top   = 'BoxR/ObstacleBoard_2.jpg';         % 顶面抠图文件（请修改为你的实际路径）
...
mask_f = ~(perspImg_front(:,:,1)<15 & perspImg_front(:,:,2)<15 & perspImg_front(:,:,3)<15);
[row_f, col_f] = find(mask_f);

h_pixel_f = max(row_f) - min(row_f);
w_pixel_f = max(col_f) - min(col_f);
H_front = L * (h_pixel_f / w_pixel_f)+30;  % Case 2 的高度补偿
...
mask_t = ~(perspImg_top(:,:,1)<15 & perspImg_top(:,:,2)<15 & perspImg_top(:,:,3)<15);

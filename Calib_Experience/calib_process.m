%%原始方法，两个校准板
%图像增强
%img=imread('TestImages/image7_1E.jpg'); up_scale=3;im_h = SRCNN_single_image(img, up_scale);
% 保存增强结果
%enhanced_name = 'image7_1E.jpg';imwrite(im_h, enhanced_name);
...
i = calib_data.n_ima;
calib_data.L(i+1)={'TestImages/image9_1.jpg'};
use_corner_find=1;
...
I1 = imread('TestImages/image9_1.jpg');
I = las_segm(I1,100);
...
%% test fo paper
%camX=-3; camY =-3; camZ =3; l_x =2; l_y =4; z_las_dist=450;
load('Omni_Calib_Results_Sim.mat');
ocam_model = calib_data.ocam_model;
I1_=imread('TestImages/image9_1.jpg');
% for las
Cent = []; 
Cent1 = []; 
I_ = las_segm(I1_,-80);
...
img1 = las_segm(I1_,108);
img1 = bwmorph(img1,'skel',Inf);
%img1=LaserFind2(I1_);%imshow(img1);
[Cent, Cent1]=intersect_blobs(img1,zz_,xx_); % Cent1,2-h, Cent2,3-v
[x,y] = mapping_points(Cent,camX,camY,camZ,...

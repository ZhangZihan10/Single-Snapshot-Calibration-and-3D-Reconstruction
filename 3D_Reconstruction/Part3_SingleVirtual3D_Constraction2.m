...
clear;
load 3DtrainedNetresnet50-10.mat;

% 1. 图像分割（仅一次性处理）
I = imread('a5.jpg');
I = imresize(I, [1920, 1920]);
C = semanticseg(I, net, 'MiniBatchSize', 32);
classes = ["platform", "obstacleboard", "cube"];
cmap = camvidColorMap; 
B = labeloverlay(I, C, 'ColorMap', cmap, 'Transparency', 0.4);
figure(1); imshow(B); title("Semantic Segmentation Result");
...
cubeOnlyImage = I;
cubeOnlyImage(~cubeMask3Channel) = 0;
figure(2); imshow(cubeOnlyImage); title('Only Cube Region');

% 分割每一块 obstacle board 并保存

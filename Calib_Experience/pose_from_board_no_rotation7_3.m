%%成功 使用像素误差版 RANSAC PnP  虚拟环境与真实环境   加坐标轴

function [C_B, R_CW, T_BC, dbg,roll_deg, pitch_deg, yaw_deg] = pose_from_board_no_rotation7_3(calib_data, kk, squareSize, use_corner_find,opts)
        ...
        if score > bestScore
            ...
            if bestScore > (N -12)                   % 所有有效点都内点
                Ishow = opts.Ishow;
                            ...
                            % 真实角点：绿色圆点
                            plot(obs_uv(idx_all,1),  obs_uv(idx_all,2),  'go', 'MarkerSize',8, 'LineWidth',3);
                        
                            % 重投影点：洋红色十字
                            plot(pred_uv_v(idx_all,1), pred_uv_v(idx_all,2), 'm+', 'MarkerSize',8, 'LineWidth',3);
                        
                            % 观测与预测的连线：黄色细线
                            for kk_=idx_all
                                line([obs_uv(kk_,1),  pred_uv_v(kk_,1)], ...
                                     [obs_uv(kk_,2),  pred_uv_v(kk_,2)], 'Color','y', 'LineWidth',1);
                            end

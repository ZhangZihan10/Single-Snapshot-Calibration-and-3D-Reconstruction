...
% 激光参数
camX = 0.81; camY = 1.72; camZ = 3.112;
%camX = 0.01; camY = 0.02; camZ = -0.012;
lasX = 0.103; lasY = -0.09;
CVsyst_rot = 0;
...
for i = 1:length(imageFiles)
    ...
    switch i
        case 1
            ...
            R2 = eul2rotm(deg2rad([yaw, pitch,  yaw2]), 'ZYZ');
        case 2
            roll = -90; pitch = 0;
                if abs(theta_deg - 0) < 1 || abs(theta_deg - 180) < 1|| abs(theta_deg - 90) < 1
                    yaw = 0;
    ...
    % 提取非黑像素区域
    mask = ~(perspectiveImg(:,:,1)<25 & perspectiveImg(:,:,2)<25 & perspectiveImg(:,:,3)<25);
...
elapsedTime = toc; % 停止计时并将时间（秒）保存到变量中

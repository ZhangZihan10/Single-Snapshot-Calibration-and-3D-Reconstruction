...
%通过鱼眼摄像头计算方块的位置
Loc=TutorialBasic1G(Client);%TutorialBasic1为使用平均值算法加姿态调整，TutorialBasic2为寻找最小值算法,TutorialBasic1G选取绿色方块

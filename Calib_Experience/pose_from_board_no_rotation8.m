...
function [C_B, R_CW, T_BC, dbg] = pose_from_board_no_rotation8(calib_data, kk, squareSize, use_corner_find,opts)
...
% ===== 计算相机最终旋转角（在可视化之前粘贴） =====
% ---- 先根据当前姿态检查光轴方向，并强制朝下：zc_world(3) < 0 ----
R_WC = R_CW.';                  % camera -> world
zc_world = R_WC(:,3);

% 也可用更稳的判据：绝大多数点应在相机前方
% Xc = (R_CW*(WP.' - C_B(:))).'; 
% need_flip = (median(Xc(:,3)) < 0);
need_flip = (zc_world(3) > 1e-4);

if need_flip
    S = diag([1 1 -1]);        % 关于 z=0 平面镜像
    R_CW = S * R_CW;           % 修正姿态
    C_B  = (S * C_B(:)).';     % 修正相机位置
    R_WC = R_CW.';             % 记得重算
    zc_world = R_WC(:,3);
...
% —— 在可视化/求欧拉角之前：正交化 R_CW，并保证 det=+1 ——
[U,~,V] = svd(R_CW);
R_CW = U*V';
if det(R_CW) < -1e-12
    % 调整符号，恢复为正规旋转
...
R_WC = R_CW.';         % camera->world（最终）
zc_world = R_WC(:,3);  % 最终光轴(世界系)

% ---- 现在再计算欧拉角（世界系 ZYX）与 heading ----
...
% 相机光轴(+Zc)在棋盘平面上的投影与方位（用已更新的 zc_world）
zproj = zc_world - dot(zc_world,[0;0;1])*[0;0;1];
if norm(zproj) > 1e-12
    zproj = zproj / norm(zproj);
    heading_deg = atan2d(zproj(2), zproj(1));
...
if opts.verbose
    ...
    % 棋盘坐标系
    local_draw_frame(eye(4), board_span*0.25, 'B');

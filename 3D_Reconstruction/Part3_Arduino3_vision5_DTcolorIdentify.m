...
%arduino=serialport("COM9",1000000);%只需要运行1次，连接端口
%arduino1=serialport("COM12",115200);%只需要运行1次，连接端口color1 = readline(arduino1);
...
%Serial.flush();
grabSend(arduino,"g");
pause(3);%并在arduino中添加一个对应函数，让g命令颜色传感器工作
color1 = readline(arduino);%读取传输到的色彩信息，真实颜色
color1 = str2double(color1);%1黑色，2白色，3红色，4绿色，5蓝色，6黄色
disp(color1);
%第2次识别颜色
grabSend(arduino,"g");
pause(3);%并在arduino中添加一个对应函数，让g命令颜色传感器工作
color2 = readline(arduino);%读取传输到的色彩信息，真实颜色
color2 = str2double(color2);%1黑色，2白色，3红色，4绿色，5蓝色，6黄色
disp(color2);

if(color1==color2)%判断两次颜色是否一样

if(color1~=2&&color1~=0)%&&color1~=empty) %判断是不是白色，来确定是否存在木块

%color = color_check(Client); % function for detecting colors

func_newObj(Client, color1);%虚拟系统中添加相同颜色的虚拟方块
%robot.plot(joints);
%方块位置
